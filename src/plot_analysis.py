...
def plot_kde_p1p99(df: pd.DataFrame) -> None:
    ...
    for race in COMPARE:
        ...
        ax.plot(xs, kde(xs), lw=2.2, color=COLORS[race], label=RACE_LABEL[race].replace("\n", " "))
        ax.fill_between(xs, kde(xs), alpha=0.12, color=COLORS[race])
    ax.set_xlabel(
        "Где участник на своей дистанции:\n"
        "0 = среди самых быстрых, 1 = среди самых медленных\n"
        "(шкала от 1-го до 99-го перцентиля времени)"
    )
    ax.set_ylabel(
        "Плотность вероятности\n"
        "(высота кривой; площадь под кривой = 1)"
    )
    ax.set_title("Где сосредоточена основная масса участников")
    ax.set_xlim(0, 1)
    ...
    add_note(
        ax,
        "СПРАВКА\n"
        "Ось X — положение финишёра на своей дистанции\n"
        "между 1% самых быстрых и 1% самых медленных.\n"
        "Ось Y — плотность вероятности: чем выше кривая,\n"
        "тем чаще встречаются такие результаты.\n"
        "Высота может быть > 1: это не проценты.\n"
        "В единицу складывается площадь под кривой.",
        "upper right",
    )
...
def plot_pace(df: pd.DataFrame) -> None:
    ...
    add_note(
        ...
        "СПРАВКА\n"
        "Ось X — темп в мин/км.\n"
        "Ось Y — плотность вероятности: выше = чаще\n"
        "такой темп. Высота может быть > 1 — это не\n"
        "доля в процентах; в единицу складывается\n"
        "площадь. Показаны данные до 99-го перцентиля.",
        "upper right",
    )
